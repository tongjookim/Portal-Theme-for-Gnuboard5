...
$featured = count($normal_items) > 0 ? $normal_items[0] : null;
$rest = array_slice($normal_items, 1);
?>
...
<?php if ($featured) {
    $ft_thumb = get_list_thumbnail($bo_table, $featured['wr_id'], $thumb_w, $thumb_h, false, true);
?>
<div class="bsk_wz_featured<?php echo ($wr_id == $featured['wr_id']) ? ' bsk_current_item' : '' ?>">
    <?php if ($is_checkbox) { ?>
    <span class="bsk_wz_chk">
        <input type="checkbox" name="chk_wr_id[]" value="<?php echo $featured['wr_id'] ?>" id="chk_wr_id_ft">
        <label for="chk_wr_id_ft"><span class="sound_only"><?php echo $featured['subject'] ?></span></label>
    </span>
    <?php } ?>
    <div class="bsk_wz_featured_link">
        <!-- 썸네일만 <a>로 감쌈 — sv_use 중첩앵커 방지 -->
        <a href="<?php echo $featured['href'] ?>" class="bsk_wz_featured_thumb_link">
            <span class="bsk_wz_featured_thumb">
                <?php if ($ft_thumb && $ft_thumb['src']) { ?>
                <img src="<?php echo $ft_thumb['src'] ?>" alt="<?php echo htmlspecialchars($ft_thumb['alt'], ENT_QUOTES) ?>">
                <?php } else { ?>
                <span class="bsk_wz_no_img"><i class="fa fa-image"></i></span>
                <?php } ?>
            </span>
        </a>
        <!-- 정보 영역: <a> 바깥에 위치해야 sv_use 사이드뷰 정상 동작 -->
        <div class="bsk_wz_featured_info">
            <?php if ($is_category && $featured['ca_name']) { ?>
            <span class="bsk_badge bsk_badge_cate"><?php echo $featured['ca_name'] ?></span>
            <?php } ?>
            <a href="<?php echo $featured['href'] ?>" class="bsk_wz_featured_tit_link">
                <h2 class="bsk_wz_featured_tit">
                    <?php if ($featured['icon_secret']) { ?><i class="fa fa-lock"></i> <?php } ?>
                    <?php echo $featured['subject'] ?>
                    <?php if ($featured['comment_cnt']) { ?><span class="bsk_list_cmt">[<?php echo $featured['comment_cnt'] ?>]</span><?php } ?>
                </h2>
            </a>
            <p class="bsk_wz_featured_excerpt"><?php echo cut_str(htmlspecialchars(strip_tags($featured['wr_content'] ?? ''), ENT_QUOTES, 'UTF-8'), 200, '...') ?></p>
            <div class="bsk_wz_featured_meta">
                <span class="sv_use"><?php echo $featured['name'] ?></span>
                <span class="bsk_meta_sep">·</span>
                <span><?php echo $featured['datetime2'] ?></span>
                <span class="bsk_meta_sep">·</span>
                <span><i class="fa fa-eye"></i> <?php echo number_format($featured['wr_hit']) ?></span>
            </div>
            <?php if ($featured['icon_new']) { ?><span class="bsk_badge bsk_badge_new">N</span><?php } ?>
        </div>
    </div>
</div>
<?php } ?>
...
<?php if (count($rest) > 0) { ?>
<ul class="bsk_wz_grid">
<?php foreach ($rest as $idx => $item) {
    $thumb = get_list_thumbnail($bo_table, $item['wr_id'], $thumb_sw, $thumb_sh, false, true);
?>
<li class="bsk_wz_item<?php echo ($wr_id == $item['wr_id']) ? ' bsk_current_item' : '' ?>">
    <?php if ($is_checkbox) { ?>
    <span class="bsk_wz_chk">
    ...
    <div class="bsk_wz_item_info">
        ...
        <a href="<?php echo $item['href'] ?>" class="bsk_wz_item_tit_link">
            <p class="bsk_wz_item_tit"><?php echo $item['subject'] ?>
